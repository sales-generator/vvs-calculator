...
} elseif ($_POST["source_site"] == Stavropol_tpp) {
    $mail_partner = "tppskovd@gmail.com";
...
$query = "SELECT
  `BASE_2017Q2`.tn_code,
  `BASE_2017Q2`.tn_name,
  `BASE_2017Q2`.tn_period,
  `BASE_2017Q2`.tn_money,
  `BASE_2017Q2`.tn_percentage,
  `BASE_2017Q2`.tn_region
FROM `BASE_2017Q2`
WHERE `BASE_2017Q2`.tn_pos = $search";
...
if($resultcheck == 0) { //если нет, командуем вывести пустой результат
  ...
  $query = "SELECT
    ...
    `BASE_2017Q2`.tn_region
  FROM `BASE_2017Q2`
  WHERE BASE_2017Q2.TN_CODE LIKE '$search%'";
  $result = mysql_query($query); // переменная с запросом
  $resultcheck = mysql_num_rows($result); //количество строк после запроса
  //полный текст ошибки
  echo "<p>По итогу предыдущего квартала в рамках Вашей товарной позиции не удалось подобрать товары, выгодные для импортозамещения. И при этом в рамках Вашей товарной группы $tn_pos_fail найдено $resultcheck позиций.</p>
  <p>Повторите запрос с иной позицией или позвоните нам по бесплатному телефону</p>
